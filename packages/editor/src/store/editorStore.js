...
export const useEditorStore = create((set, get) => ({
    ...
    bringForward: (id) => set((s) => {
        ...
        if (!w)
            return {};
        const newWidgets = s.schema.widgets.map((ww) => ww.id === id ? { ...ww, geometry: { ...ww.geometry, zIndex: ww.geometry.zIndex + 1 } } : ww);
        return pushHistory(s, { ...s.schema, widgets: newWidgets });
    }),
    sendBackward: (id) => set((s) => {
        ...
        if (!w)
            return {};
        const newWidgets = s.schema.widgets.map((ww) => ww.id === id ? { ...ww, geometry: { ...ww.geometry, zIndex: Math.max(1, ww.geometry.zIndex - 1) } } : ww);
        return pushHistory(s, { ...s.schema, widgets: newWidgets });
    }),
