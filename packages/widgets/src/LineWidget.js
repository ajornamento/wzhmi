...
export class LineWidget extends BaseWidget {
    ...
    render() {
        ...
        if (lineStyle === 'dashed')
            path.setAttribute('stroke-dasharray', `${lineWidth * 5} ${lineWidth * 3}`);
        if (lineStyle === 'dotted')
            path.setAttribute('stroke-dasharray', `${lineWidth} ${lineWidth * 3}`);
        if (arrowEnd)
            path.setAttribute('marker-end', `url(#${markerId}-e)`);
        if (arrowStart)
            path.setAttribute('marker-start', `url(#${markerId}-s)`);
        this._path = path;
    ...
    startFlow(color) {
        this.stopFlow();
        if (!this._flowPath || !this._widget)
            return;
        ...
        this._flowInterval = setInterval(() => {
            ...
            if (this._flowOffset <= -period)
                this._flowOffset += period;
