...
export class WorkstationWidget extends BaseWidget {
  private _statusLight: SVGCircleElement | null = null;
  private _monitorScreen: SVGRectElement | null = null;
  private _surface: SVGRectElement | null = null;
  ...
  protected render() {
    ...
    frame.setAttribute('width', String(W - 2)); frame.setAttribute('height', String(H - 2));
    frame.setAttribute('fill', '#1c1c2c'); frame.setAttribute('stroke', dark);
    frame.setAttribute('stroke-width', '1.5'); frame.setAttribute('rx', '3');
    svg.appendChild(frame);

    // 작업 테이블 상판
    const sy = H * 0.55, sh = H * 0.07, sx = W * 0.08, sw = W * 0.84;
    const surface = document.createElementNS(ns, 'rect');
    surface.setAttribute('x', String(sx)); surface.setAttribute('y', String(sy));
    surface.setAttribute('width', String(sw)); surface.setAttribute('height', String(sh));
    surface.setAttribute('fill', mid); surface.setAttribute('rx', '2');
    ...
    for (const lx of [sx + sw * 0.1, sx + sw * 0.83]) {
      ...
      leg.setAttribute('x', String(lx)); leg.setAttribute('y', String(sy + sh));
      leg.setAttribute('width', String(legW)); leg.setAttribute('height', String(H * 0.32));
    ...
    monFrame.setAttribute('x', String(mx)); monFrame.setAttribute('y', String(my));
    monFrame.setAttribute('width', String(mw)); monFrame.setAttribute('height', String(mh));
    monFrame.setAttribute('fill', '#111'); monFrame.setAttribute('stroke', dark);
    monFrame.setAttribute('stroke-width', '1.5'); monFrame.setAttribute('rx', '2');
    svg.appendChild(monFrame);

    // 모니터 화면
    const screen = document.createElementNS(ns, 'rect');
    screen.setAttribute('x', String(mx + 2)); screen.setAttribute('y', String(my + 2));
    screen.setAttribute('width', String(mw - 4)); screen.setAttribute('height', String(mh - 4));
    ...
    stand.setAttribute('x', String(mx + mw * 0.38)); stand.setAttribute('y', String(my + mh));
    stand.setAttribute('width', String(mw * 0.24)); stand.setAttribute('height', String(H * 0.03));
    stand.setAttribute('fill', dark);
    svg.appendChild(stand);

    // 테이블 위 도구
    const tool = document.createElementNS(ns, 'rect');
    tool.setAttribute('x', String(sx + sw * 0.72)); tool.setAttribute('y', String(sy - H * 0.12));
    tool.setAttribute('width', String(sw * 0.07)); tool.setAttribute('height', String(H * 0.12));
    tool.setAttribute('fill', color); tool.setAttribute('rx', '2');
    svg.appendChild(tool);

    // 상태 표시등
    const lr = Math.max(Math.min(W, H) * 0.06, 4);
    const light = document.createElementNS(ns, 'circle');
    light.setAttribute('cx', String(W - 8)); light.setAttribute('cy', '8');
    light.setAttribute('r', String(lr));
    light.setAttribute('fill', '#333'); light.setAttribute('stroke', '#555');
