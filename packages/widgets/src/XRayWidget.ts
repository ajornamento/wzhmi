...
export class XRayWidget extends BaseWidget {
  private _statusLed: SVGCircleElement | null = null;
  private _beamEls: SVGLineElement[] = [];
  private _scanEl: SVGRectElement | null = null;
  private _statusText: SVGTextElement | null = null;
  ...
  protected render() {
    ...
    housing.setAttribute('x', String(bx)); housing.setAttribute('y', String(by));
    housing.setAttribute('width', String(bw)); housing.setAttribute('height', String(bh));
    housing.setAttribute('rx', '3'); housing.setAttribute('fill', '#282838');
    housing.setAttribute('stroke', '#445566'); housing.setAttribute('stroke-width', '1.5');
    svg.appendChild(housing);

    const hHL = document.createElementNS(ns, 'rect');
    hHL.setAttribute('x', String(bx + 1)); hHL.setAttribute('y', String(by + 1));
    hHL.setAttribute('width', String(bw - 2)); hHL.setAttribute('height', '3');
    hHL.setAttribute('rx', '2'); hHL.setAttribute('fill', 'rgba(255,255,255,0.07)');
    svg.appendChild(hHL);

    // 상태 패널
    const panel = document.createElementNS(ns, 'rect');
    panel.setAttribute('x', String(bx + 2)); panel.setAttribute('y', String(by + 2));
    panel.setAttribute('width', String(bw - 4)); panel.setAttribute('height', String(panelH - 1));
    panel.setAttribute('rx', '2'); panel.setAttribute('fill', '#181828');
    panel.setAttribute('stroke', '#334455'); panel.setAttribute('stroke-width', '0.5');
    ...
    led.setAttribute('cx', String(ledCx)); led.setAttribute('cy', String(ledCy));
    led.setAttribute('r', String(ledR)); led.setAttribute('fill', '#222');
    this._statusLed = led as SVGCircleElement;
    svg.appendChild(led);

    const ledHL = document.createElementNS(ns, 'circle');
    ledHL.setAttribute('cx', String(ledCx - 1)); ledHL.setAttribute('cy', String(ledCy - 1));
    ledHL.setAttribute('r', '1.2'); ledHL.setAttribute('fill', 'rgba(255,255,255,0.35)');
    ...
    this._statusText = statusText as SVGTextElement;
    svg.appendChild(statusText);

    // 벨트 입구 슬롯 (왼쪽)
    const leftSlot = document.createElementNS(ns, 'rect');
    leftSlot.setAttribute('x', String(bx)); leftSlot.setAttribute('y', String(slotY));
    leftSlot.setAttribute('width', String(slotW + 1)); leftSlot.setAttribute('height', String(slotH));
    leftSlot.setAttribute('fill', '#0a0a12');
    svg.appendChild(leftSlot);

    // 벨트 출구 슬롯 (오른쪽)
    const rightSlot = document.createElementNS(ns, 'rect');
    rightSlot.setAttribute('x', String(bx + bw - slotW - 1)); rightSlot.setAttribute('y', String(slotY));
    rightSlot.setAttribute('width', String(slotW + 1)); rightSlot.setAttribute('height', String(slotH));
    rightSlot.setAttribute('fill', '#0a0a12');
    svg.appendChild(rightSlot);

    // 하우징 외곽선 재드로우 (슬롯 위에 테두리)
    const border = document.createElementNS(ns, 'rect');
    border.setAttribute('x', String(bx)); border.setAttribute('y', String(by));
    border.setAttribute('width', String(bw)); border.setAttribute('height', String(bh));
    border.setAttribute('rx', '3'); border.setAttribute('fill', 'none');
    border.setAttribute('stroke', '#445566'); border.setAttribute('stroke-width', '1.5');
    svg.appendChild(border);

    // 슬롯 경계선
    for (const sx of [bx + slotW, bx + bw - slotW]) {
      const guide = document.createElementNS(ns, 'line');
      guide.setAttribute('x1', String(sx)); guide.setAttribute('y1', String(slotY));
      guide.setAttribute('x2', String(sx)); guide.setAttribute('y2', String(slotY + slotH));
      guide.setAttribute('stroke', '#445566'); guide.setAttribute('stroke-width', '1');
      svg.appendChild(guide);
    }

    // 챔버 내부 검사 영역
    const chamber = document.createElementNS(ns, 'rect');
    chamber.setAttribute('x', String(chamberX)); chamber.setAttribute('y', String(chamberY));
    chamber.setAttribute('width', String(chamberW)); chamber.setAttribute('height', String(chamberH));
    chamber.setAttribute('rx', '2'); chamber.setAttribute('fill', '#080812');
    chamber.setAttribute('stroke', '#1a2a44'); chamber.setAttribute('stroke-width', '0.5');
    ...
    for (let i = 0; i < beamCount; i++) {
      ...
      beam.setAttribute('x1', String(srcX)); beam.setAttribute('y1', String(srcY));
      beam.setAttribute('x2', String(endX)); beam.setAttribute('y2', String(endY));
      beam.setAttribute('stroke', '#001a44'); beam.setAttribute('stroke-width', '0.8');
      beam.setAttribute('opacity', '0.5');
      this._beamEls.push(beam as SVGLineElement);
      svg.appendChild(beam);
    }

    // 빔 소스 포인트
    const srcDot = document.createElementNS(ns, 'circle');
    srcDot.setAttribute('cx', String(srcX)); srcDot.setAttribute('cy', String(srcY));
    srcDot.setAttribute('r', '2.5'); srcDot.setAttribute('fill', '#001133');
    svg.appendChild(srcDot);

    // 스캔 글로우 오버레이 (애니메이션용)
    const scanEl = document.createElementNS(ns, 'rect');
    scanEl.setAttribute('x', String(chamberX + 1)); scanEl.setAttribute('y', String(srcY - 2));
    scanEl.setAttribute('width', String(chamberW - 2)); scanEl.setAttribute('height', String(chamberH - srcY + chamberY - 1));
    scanEl.setAttribute('fill', 'rgba(0,80,200,0.12)');
    scanEl.setAttribute('opacity', '0');
    this._scanEl = scanEl as SVGRectElement;
    svg.appendChild(scanEl);

    // 챔버 내 벨트 영역 표시
    const beltInner = document.createElementNS(ns, 'rect');
    beltInner.setAttribute('x', String(chamberX)); beltInner.setAttribute('y', String(slotY));
    beltInner.setAttribute('width', String(chamberW)); beltInner.setAttribute('height', String(slotH));
    beltInner.setAttribute('fill', '#111118'); beltInner.setAttribute('opacity', '0.85');
    svg.appendChild(beltInner);

    const beltHL = document.createElementNS(ns, 'rect');
    beltHL.setAttribute('x', String(chamberX)); beltHL.setAttribute('y', String(slotY));
    beltHL.setAttribute('width', String(chamberW)); beltHL.setAttribute('height', '2');
