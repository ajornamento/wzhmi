...
export class HopperWidget extends BaseWidget {
  private _fillPoly: SVGPolygonElement | null = null;
  ...
  protected render() {
    ...
    this._W = W; this._topY = topY; this._outletY = outletY;
    this._outletX = outletX; this._outletW = outletW; this._pad = pad;
    ...
    rim.setAttribute('x', String(pad - 1)); rim.setAttribute('y', String(topY - 3));
    rim.setAttribute('width', String(W - pad * 2 + 2)); rim.setAttribute('height', '5');
    ...
    for (const lx of [outletX - legW * 1.5, outletX + outletW + legW * 0.5]) {
      ...
      leg.setAttribute('x', String(lx)); leg.setAttribute('y', String(outletY));
      leg.setAttribute('width', String(legW)); leg.setAttribute('height', String(legH));
    ...
    pipe.setAttribute('x', String(outletX)); pipe.setAttribute('y', String(outletY));
    pipe.setAttribute('width', String(outletW)); pipe.setAttribute('height', String(outletPipeH));
    pipe.setAttribute('fill', dark);
    pipe.setAttribute('stroke', adj(color, -20)); pipe.setAttribute('stroke-width', '1');
