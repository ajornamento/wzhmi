...
export class ReactorWidget extends BaseWidget {
  private _agitator: SVGGElement | null = null;
  private _agitInterval: ReturnType<typeof setInterval> | null = null;
  private _agitAngle = 0;
  private _cx = 0;
  private _cy = 0;
  ...
  protected render() {
    ...
    nL.setAttribute('x', '0'); nL.setAttribute('y', String(H * 0.35));
    nL.setAttribute('width', String(bx + 1)); nL.setAttribute('height', String(nH));
    nL.setAttribute('fill', dark);
    svg.appendChild(nL);

    // 우측 노즐
    const nR = document.createElementNS(ns, 'rect');
    nR.setAttribute('x', String(bx + bw - 1)); nR.setAttribute('y', String(H * 0.58));
    nR.setAttribute('width', String(bx + 1)); nR.setAttribute('height', String(nH));
    ...
    for (const [off, c] of [['0%', light], ['40%', color], ['100%', dark]] as [string, string][]) {
    ...
    body.setAttribute('x', String(bx)); body.setAttribute('y', String(topY));
    ...
    topDome.setAttribute('cx', String(cx)); topDome.setAttribute('cy', String(topY));
    topDome.setAttribute('rx', String(bw / 2)); topDome.setAttribute('ry', String(domeRy));
    topDome.setAttribute('fill', light);
    svg.appendChild(topDome);

    // 하단 돔
    const botDome = document.createElementNS(ns, 'ellipse');
    botDome.setAttribute('cx', String(cx)); botDome.setAttribute('cy', String(botY));
    botDome.setAttribute('rx', String(bw / 2)); botDome.setAttribute('ry', String(domeRy));
    botDome.setAttribute('fill', dark);
    svg.appendChild(botDome);

    // 본체 외곽선
    const outline = document.createElementNS(ns, 'rect');
    outline.setAttribute('x', String(bx)); outline.setAttribute('y', String(topY));
    outline.setAttribute('width', String(bw)); outline.setAttribute('height', String(bh));
    outline.setAttribute('fill', 'none');
    outline.setAttribute('stroke', adj(color, -15)); outline.setAttribute('stroke-width', '1.5');
    ...
    shaft.setAttribute('x1', String(cx)); shaft.setAttribute('y1', String(topY + domeRy));
    shaft.setAttribute('x2', String(cx)); shaft.setAttribute('y2', String(botY - domeRy));
    shaft.setAttribute('stroke', dark); shaft.setAttribute('stroke-width', '2');
    agit.appendChild(shaft);

    for (const fy of [H * 0.3, H * 0.52, H * 0.72]) {
      const blade = document.createElementNS(ns, 'line');
      blade.setAttribute('x1', String(cx - bw * 0.38)); blade.setAttribute('y1', String(fy));
      blade.setAttribute('x2', String(cx + bw * 0.38)); blade.setAttribute('y2', String(fy + H * 0.05));
      blade.setAttribute('stroke', adj(color, 15)); blade.setAttribute('stroke-width', '2.5');
